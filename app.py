...
if new_game:
    st.session_state.attempts = 0
    # FIX: Difficulty ranges are not hardcoded
    st.session_state.secret = random.randint(low, high)
    # FIX: Status actually shows playing so game can reset and history is reset
    st.session_state.status = "playing"
    st.session_state.history = []
...
if submit:
    ...
    if not ok:
        ...
        # FIX: Secret was intentionally turned into a string 
        # after every even number of attempts
